mcApp.controller('service4Ctrl', function($scope, $cookies, $state, InputService, DataService, DeleteService) {
	...
	$scope.removeEvent = function(index) {
		console.log(index);
		var temp = $scope.wellnessEventsArr;
		$scope.wellnessEventsArr = [];
		for (var i = 0; i < temp.length; i++) {
			if (i !== index) {
				$scope.wellnessEventsArr.push(temp[i]);
			}
		}
	...
	$scope.doneEditing = function(index, that) {
		...
		that.openEdit = !that.openEdit;
		var event = { name: "", container: "", location: {address: "", city: "", state: "", zipcode: ""}, contact: "", datetime: {year: "", month: "", date: "", hour: "", min: ""}, dbid: $scope.wellnessEventsArr[index].dbid };
		var yr;
		var mo;
		var date;
		var hr;
